# Sequências Rápidas — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Adicionar uma seção de protocolos clínicos de emergência com 10 sequências rápidas acessíveis via URL própria (`/protocolos` e `/protocolos/:slug`).

**Architecture:** Conteúdo estático em `frontend/src/data/protocolos.js`; duas novas páginas React sem chamadas de API; link fixo na Sidebar existente. Sem backend.

**Tech Stack:** React 18, React Router 6, SCSS Modules, Vitest + Testing Library
...
|---|---|---|
| Criar | `frontend/src/data/protocolos.js` | Fonte única dos 10 protocolos |
| Criar | `frontend/src/pages/Protocolos.jsx` | Página lista `/protocolos` |
| Criar | `frontend/src/pages/Protocolos.module.scss` | Estilos da lista |
| Criar | `frontend/src/pages/ProtocoloDetalhe.jsx` | Página detalhe `/protocolos/:slug` |
| Criar | `frontend/src/pages/ProtocoloDetalhe.module.scss` | Estilos do detalhe |
| Criar | `frontend/src/__tests__/protocolos.data.test.js` | Testa integridade dos dados |
| Criar | `frontend/src/__tests__/Protocolos.test.jsx` | Testa página lista |
| Criar | `frontend/src/__tests__/ProtocoloDetalhe.test.jsx` | Testa página detalhe |
| Modificar | `frontend/src/App.jsx` | Adiciona 2 rotas novas |
| Modificar | `frontend/src/components/Sidebar.jsx` | Adiciona link "Protocolos" |
| Modificar | `frontend/src/components/Sidebar.module.scss` | Estilo do link |

---

## Task 1: Criar arquivo de dados dos protocolos

**Files:**
- Create: `frontend/src/data/protocolos.js`
...
```bash
git add frontend/src/data/protocolos.js
git commit -m "feat(protocolos): adiciona dados estáticos dos 10 protocolos clínicos"
```

---

## Task 2: Testes dos dados + testes das páginas (TDD — escrever antes da UI)

**Files:**
- Create: `frontend/src/__tests__/protocolos.data.test.js`
- Create: `frontend/src/__tests__/Protocolos.test.jsx`
- Create: `frontend/src/__tests__/ProtocoloDetalhe.test.jsx`

- [ ] **Step 1: Escrever teste de integridade dos dados**

```js
// frontend/src/__tests__/protocolos.data.test.js
...
- [ ] **Step 2: Escrever teste da página lista**

```jsx
// frontend/src/__tests__/Protocolos.test.jsx
import { describe, it, expect } from 'vitest';
...
describe('Protocolos (lista)', () => {
  ...
  it('renderiza 10 cards de protocolos', () => {
    ...
    // 10 cards + 1 link de volta (← Conduta)
...
- [ ] **Step 3: Escrever teste da página detalhe**

```jsx
// frontend/src/__tests__/ProtocoloDetalhe.test.jsx
import { describe, it, expect } from 'vitest';
import { render, screen } from '@testing-library/react';
...
- [ ] **Step 4: Rodar testes para confirmar que falham (componentes ainda não existem)**

```bash
cd frontend && npm test -- --run 2>&1 | tail -20
```

Esperado: erros de "Cannot find module '../pages/Protocolos'" e "Cannot find module '../pages/ProtocoloDetalhe'"

- [ ] **Step 5: Commit dos testes**

```bash
git add frontend/src/__tests__/protocolos.data.test.js frontend/src/__tests__/Protocolos.test.jsx frontend/src/__tests__/ProtocoloDetalhe.test.jsx
git commit -m "test(protocolos): adiciona testes de dados, lista e detalhe"
```

---

## Task 3: Página lista `/protocolos`

**Files:**
- Create: `frontend/src/pages/Protocolos.jsx`
- Create: `frontend/src/pages/Protocolos.module.scss`

- [ ] **Step 1: Criar a página lista**

```jsx
// frontend/src/pages/Protocolos.jsx
...
- [ ] **Step 2: Criar os estilos da lista**

```scss
// frontend/src/pages/Protocolos.module.scss
...
// Cores por categoria
.catViaAerea    { background: #dbeafe; color: #1e40af; }
.catCardiovascular { background: #fee2e2; color: #991b1b; }
.catNeurologico { background: #ede9fe; color: #5b21b6; }
.catMetabolico  { background: #ffedd5; color: #9a3412; }
.catInfeccioso  { background: #dcfce7; color: #166534; }
...
- [ ] **Step 3: Rodar testes da lista**

```bash
cd frontend && npm test -- --run --reporter=verbose 2>&1 | grep -E "(PASS|FAIL|✓|✗|Protocolos)"
```

Esperado: testes de `Protocolos.test.jsx` passando (4 testes)

- [ ] **Step 4: Commit**

```bash
git add frontend/src/pages/Protocolos.jsx frontend/src/pages/Protocolos.module.scss
git commit -m "feat(protocolos): adiciona página lista /protocolos"
```

---

## Task 4: Página detalhe `/protocolos/:slug`

**Files:**
- Create: `frontend/src/pages/ProtocoloDetalhe.jsx`
- Create: `frontend/src/pages/ProtocoloDetalhe.module.scss`

- [ ] **Step 1: Criar a página detalhe**
...
- [ ] **Step 2: Criar os estilos do detalhe**

```scss
// frontend/src/pages/ProtocoloDetalhe.module.scss
@use '../styles/variables' as *;
...
- [ ] **Step 3: Rodar testes do detalhe**

```bash
cd frontend && npm test -- --run --reporter=verbose 2>&1 | grep -E "(PASS|FAIL|✓|✗|ProtocoloDetalhe|protocolos)"
```

Esperado: todos os testes de `ProtocoloDetalhe.test.jsx` e `protocolos.data.test.js` passando

- [ ] **Step 4: Commit**

```bash
git add frontend/src/pages/ProtocoloDetalhe.jsx frontend/src/pages/ProtocoloDetalhe.module.scss
git commit -m "feat(protocolos): adiciona página detalhe /protocolos/:slug"
```

---

## Task 5: Wiring — rotas no App.jsx e link na Sidebar
...
- [ ] **Step 1: Adicionar as rotas em App.jsx**

Localizar o bloco de rotas privadas em `frontend/src/App.jsx`. Adicionar as duas novas rotas logo antes de `<Route path="*" ...>`:
...
// Adicionar dentro de <Routes>, antes do wildcard:
<Route
  path="/protocolos"
  element={
    <PrivateRoute>
...
- [ ] **Step 2: Adicionar estilo do link na Sidebar.module.scss**

Adicionar no final do arquivo `frontend/src/components/Sidebar.module.scss`:
...
- [ ] **Step 3: Adicionar o link na Sidebar.jsx**

No arquivo `frontend/src/components/Sidebar.jsx`:

1. Adicionar import no topo:
```jsx
import { Link, useLocation } from 'react-router-dom';
```
(Nota: `Link` já pode estar importado via `react-router-dom` — verificar e mesclar com o import existente)

2. Adicionar dentro do componente, logo após o hook `useAuth`:
```jsx
const location = useLocation();
```

3. Adicionar o link entre o botão `newCase` e o `searchInput`:
```jsx
<Link
  to="/protocolos"
...
Esperado: todos os testes passando (incluindo os existentes — garantir nenhuma regressão)

- [ ] **Step 5: Commit final**

```bash
git add frontend/src/App.jsx frontend/src/components/Sidebar.jsx frontend/src/components/Sidebar.module.scss
git commit -m "feat(protocolos): conecta rotas e link na sidebar"
```

---

## Task 6: Push e verificação

- [ ] **Step 1: Push para remote**

```bash
git pull origin main && git push origin main
```

- [ ] **Step 2: Verificar manualmente os 3 fluxos críticos**

1. Login → sidebar exibe "⚡ Protocolos" → clicar → `/protocolos` carrega grid com 10 cards
2. Clicar em "Sequência Rápida de Intubação" → URL muda para `/protocolos/sri` → protocolo renderiza com fases, badges de droga (azul) e alertas (amarelo)
3. Acessar `/protocolos/slug-invalido` → redireciona para `/protocolos` sem erro
